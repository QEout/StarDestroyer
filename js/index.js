...
var Ship = (function() {
    ...
    var def = {
        ...
        update: function() {
            this.vel.add(this.thrust);
            this.pos.add(this.vel);
            if (this.vel.getLength() > 6) this.vel.setLength(6);
            this.speed = (this.vel.getLength() * 100) >> 0;
            ++this.bulletDelay;
        ...
        shoot: function() {
            if (this.bulletDelay > 8) {
                ...
                // this.bulletDelay = 0;
                this.bulletDelay = 10 * this.level - 100;
                if (this.bulletDelay > 0) this.bulletDelay = 0;
            }
        },
        upgrade: function() {
            this.level = checkProfit / 10;
            this.speeder = checkProfit / 500;
            if (this.speeder > 0.5) this.speeder = 0.5;
        }
    };
...
function updateState() {
    document.getElementById('profit').innerHTML = checkProfit;
    document.getElementById('speed').innerHTML = ship.speed;
    document.getElementById('level').innerHTML = ship.level;
}
function updateShip() {
    ship.update();
    if (ship.idle) return;
    ...
    if (keyUp) {
        ship.thrust.setLength(ship.speeder);
...
function generateThrustParticle() {
    ...
    p.vel.setLength(ship.speed / 20 / p.radius);
    if (ship.speed / 20 > 4) p.vel.setLength(4 / p.radius);
...
function checkDistanceAttract(obj1, obj2) {
    var vx = obj1.pos.getX() - obj2.pos.getX();
    var vy = obj1.pos.getY() - obj2.pos.getY();
    var vec = Vec2D.create(vx, vy);
    var dist = 0;
    ...
    if (vec.getLength() < obj1.radius + obj2.radius + dist) {
        renderAttract(obj1, obj2);
        obj2.thrust.setAngle(angle);
        obj2.thrust.setLength(0.5);
        // if(obj2==ship) obj2.thrust.setLength(0.05);
    }
}
// function generateAttract(obj1,obj2){
//     var vx = obj1.pos.getX() - obj2.pos.getX();
//     var vy = obj1.pos.getY() - obj2.pos.getY();
//     var angle = Math.atan2(vy, vx);
//     obj2.thrust.setLength(0.5);
...
function renderAttract(obj1, obj2) {
    context.beginPath();
    context.lineWidth = 1;
    context.strokeStyle = obj1.color;
    context.moveTo(obj1.pos.getX(), obj1.pos.getY());
    context.lineTo(obj2.pos.getX(), obj2.pos.getY());
    context.stroke();
}
